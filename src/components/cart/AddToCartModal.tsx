...
const AddToCartModal: React.FC<AddToCartModalProps> = ({
  ...
  // Verificar se é uma assinatura (categoria "Serviços")
  const isSubscription = game.category === 'Serviços';
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-[9999]">
      <div className="bg-white rounded-lg shadow-xl w-full max-w-sm overflow-hidden">
        ...
        <div className="p-4">
          ...
          {isSubscription ? (
            // Para assinaturas, apenas um botão com o preço
            <div className="space-y-3">
              <button
                onClick={handleAddSubscription}
                className="w-full py-3 px-4 rounded flex items-center justify-between bg-primary text-white hover:bg-primary/90 transition-colors"
              >
                <span>Adicionar ao Carrinho</span>
                <span className="font-bold">
                  {formatPrice(game.primaryValue)}
                </span>
              </button>
            </div>
          ) : (
            // Para jogos, opções de primária e secundária
            <>
              ...
              <div className="space-y-3">
                ...
                >
                  ...
                  <span className="font-bold">
                    {formatPrice(game.secondaryValue)}
                  </span>
                </button>
              </div>
            </>
